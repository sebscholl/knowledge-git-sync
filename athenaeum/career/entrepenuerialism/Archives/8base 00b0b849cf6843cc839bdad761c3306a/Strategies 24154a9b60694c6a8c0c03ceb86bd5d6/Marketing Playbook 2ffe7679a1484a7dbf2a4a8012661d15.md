...
(1) Who is our target audience?
	(a) "Web and mobile developers building end-user applications."	

	*THINK* How many of them have we talked to one-on-one?

(2) What problem are we solving for our target audience?
	(a) High level: "Too much expertise, across too many disciplines, is too often required to build great software."
	(b) Product level: "When developing applications, too much time gets spent re-inventing the wheel with systems that just need to work." 
	(c) Feature level:
		(I) CRUD Boilerplate 
		(II) Database connections
		(III) Scalability is mysterious
		(IV) File management
		(V) User management
		(VI) ect...
